...
namespace Kamgam.UGUIBlurredBackground
{
    public class BlurredBackgroundPassURP : ScriptableRenderPass
    {
        ...
        public const string ShaderName = "Kamgam/UGUI/URP/Blur Shader";
        ...
        public RTHandle RenderTargetHandleA
        {
            get
            {
                // Release if render texture is invalid
                bool rtIsInvalid = (_renderTargetBlurredA == null || !_renderTargetBlurredA.IsCreated());
                if (_renderTargetHandleA != null && rtIsInvalid)
                {
                    RTHandles.Release(_renderTargetHandleA);
                    _renderTargetHandleA = null;
                }
        ...
        public RTHandle RenderTargetHandleB
        {
            ...
            {
                // Release if render texture is invalid
                bool rtIsInvalid = (_renderTargetBlurredB == null || !_renderTargetBlurredB.IsCreated());
                if (_renderTargetHandleB != null && rtIsInvalid)
                {
                    RTHandles.Release(_renderTargetHandleB);
                    _renderTargetHandleB = null;
                }
        ...
        public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
        {
            ...
            using (var builder = renderGraph.AddRasterRenderPass<CopyPassData>("UGUIBlurredBackground_CopyColor", out var passData, profilingSampler))
            {
            ...
            using (var builder = renderGraph.AddRasterRenderPass<BlurPassData>("UGUIBlurredBackground_BlurHori", out var passData, profilingSampler))
            {
            ...
            // Blur vertical pass
            using (var builder = renderGraph.AddRasterRenderPass<BlurPassData>("UGUIBlurredBackground_BlurVerti", out var passData, profilingSampler))
            {
                passData.material = Material;
